...
twtable <- c(cocor(~prop_good + sent_roll3_weighted_derstandard|prop_good + sent_roll3_weighted_shifted1_twitter,data=cordt)@hittner2003$p.value,
             ...
             cocor(~prop_good + sentA_pos_roll3_weighted_derstandard|prop_good + sentA_neg_roll3_weighted_derstandard,data=cordt)@hittner2003$p.value,
             cocor(~prop_good + sentA_pos_roll3_weighted_shifted1_twitter|prop_good + sentA_neg_roll3_weighted_shifted1_twitter,data=cordt)@hittner2003$p.value,
             cocor(~prop_good + sentB_pos_roll3_weighted_derstandard|prop_good + sentB_neg_roll3_weighted_derstandard,data=cordt)@hittner2003$p.value,
             cocor(~prop_good + sentB_pos_roll3_weighted_shifted1_twitter|prop_good + sentB_neg_roll3_weighted_shifted1_twitter,data=cordt)@hittner2003$p.value)

twtable_diff <- c(cor(cordt$prop_good,cordt$sent_roll3_weighted_derstandard) - cor(cordt$prop_good,cordt$sent_roll3_weighted_shifted1_twitter),
                  cor(cordt$prop_good,cordt$sentB_roll3_weighted_derstandard)-cor(cordt$prop_good,cordt$sentA_roll3_weighted_derstandard),
                  cor(cordt$prop_good,cordt$sentB_roll3_weighted_shifted1_twitter) - cor(cordt$prop_good,cordt$sentA_roll3_weighted_shifted1_twitter),
                  cor(cordt$prop_good,cordt$sentA_pos_roll3_weighted_derstandard) - cor(cordt$prop_good,cordt$sentA_neg_roll3_weighted_derstandard),
                  cor(cordt$prop_good,cordt$sentA_pos_roll3_weighted_shifted1_twitter) - cor(cordt$prop_good,cordt$sentA_neg_roll3_weighted_shifted1_twitter),
                  cor(cordt$prop_good,cordt$sentB_pos_roll3_weighted_derstandard) - cor(cordt$prop_good,cordt$sentB_neg_roll3_weighted_derstandard),
                  cor(cordt$prop_good,cordt$sentB_pos_roll3_weighted_shifted1_twitter) - cor(cordt$prop_good,cordt$sentB_neg_roll3_weighted_shifted1_twitter))


twtable_diff_names <- paste(round(twtable_diff,2),c("(derstandard)","(GS)","(GS)","(LIWC pos)","(LIWC pos)","(GS pos)","(GS pos)"))
...
colnames(tabledt) <- c("p Value","Difference")
...
print(ggplot(corsdt,aes(x=t)) + geom_density(fill="red") + ggtitle(twtable_names[i]) + annotate("text",label=paste0('95% CI\n [',paste0(round(apply(cors$t,2, quantile, c(.025, 0.975),na.rm=T)[,1],3),collapse = ","),']'),x = -1.8, y = max(d$y)-0.5) + scale_x_continuous(limits = c(-2, 0.5)))
}
```
